...
mixin MovementV2 on GameComponent {
  ...
  void moveLeftOneShot(double speed) {
    position += lastDisplacement = Vector2(-speed, 0) * dtUpdate;
  }

  void moveRightOneShot(double speed) {
    position += lastDisplacement = Vector2(speed, 0) * dtUpdate;
  }

  void moveUpOneShot(double speed) {
    position += lastDisplacement = Vector2(0, -speed) * dtUpdate;
  }

  void moveDownOneShot(double speed) {
    position += lastDisplacement = Vector2(0, speed) * dtUpdate;
  }
  ...
  bool moveFromAngle(double speed, double angle) {
    ...
    velocity = BonfireUtil.diffMovePointByAngle(
      center,
      speed,
      angle,
    );
  ...
  void update(double dt) {
    super.update(dt);
    position += lastDisplacement = velocity * dt;
    dtUpdate = dt;
  }
  ...
  void _updateDirectionBYAngle(double angle) {
